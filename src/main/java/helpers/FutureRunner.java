...
public class FutureRunner extends AbstractFuture<Void> {
    ...
    protected <T> void run(AsyncCallable<T> request, Consumer<T> response,
            Consumer<Exception> perRequestResponseCatch) {
    ...
    protected <T> void run(AsyncCallable<T> request, Consumer<T> response, Consumer<Exception> perRequestResponseCatch, Runnable perRequestResponseFinally) {
        synchronized (lock) {
            ...
            try {
                ...
                // invoke request
                //
                ListenableFuture<T> responseFuture = request.call(); // throws
                futures.add(responseFuture);
                responseFuture.addListener(() -> {
                    synchronized (lock) {
                        --running;
                        futures.remove(responseFuture);
                        try {
                            //
                            // invoke response
                            //
                            response.accept(responseFuture.get()); // throws
                        } catch (Exception e1) {
                            try {
                ...
                try {
                    try {
                        perRequestResponseCatch.accept(e1);
                    } catch (Exception e2) {
                        setException(e2); // fail fast
                    }
                } finally {
